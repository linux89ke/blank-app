...
with st.sidebar:
    ...
    # ── Reference data: always loaded from disk, no upload needed ─────────────
    ...
    if use_ai_matching:
        ...
        if not GROQ_AVAILABLE:
            ...
            # Paste-friendly API key input with show/hide toggle
            ...
            groq_api_key = st.text_input(
                ...
                help="Paste with Ctrl+V / Cmd+V. Tick the checkbox above to see what you typed.",

            )

            if groq_api_key and not groq_api_key.startswith("gsk_"):

                st.warning("Groq keys usually start with `gsk_` — double-check your key.")
...
if queries:
    ...
    if all_result_frames:
        ...
        # ── Category & Brand Preview with manual override dropdowns ────────────
        ...
        if df_cat is not None:
            ...
            with st.expander(f"{mode_label} — Category & Brand Matching Preview", expanded=False):
                ...
                # Build sorted list of all available export categories for dropdowns
                ...
                # Session-state key for manual overrides: maps row index -> {primary, additional}
                ...
                st.markdown(
                    ...
                    "Manual picks flow into the downloaded template."
                ...
                for i, (_, prow) in enumerate(combined.iterrows()):
                    ...
                    c1.markdown(
                        ...
                        f"**{prow.get('sku_num_sku_r3', '')}**  \n"

                        f"{str(prow.get('product_name', ''))[:55]}"
                    ...
                    # Persist overrides to session state
            ...
            if df_cat is None:
                ...
                try:
                    ...
                    tpl_bytes = build_template(
                        ...
                        use_ai=True,           # always pass merged list
