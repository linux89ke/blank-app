...
class CategoryMatcherEngine:
    def __init__(self, db_path="cat_learning.db"):
      ...
      def predict_batch(self, names: list) -> dict:
        """
        Score a list of product names in a single TF-IDF transform.

        Returns:
            {name: (predicted_category_path, confidence_float)}

        This is 10-50× faster than calling get_category_with_boost() per row
        because sklearn's transform() is vectorised over all rows at once.
        """
        if not self._tfidf_built or not names:
            return {}

        # Preprocess all names at once
        processed = [self._preprocess(n) for n in names]

        # Single transform + predict_proba call
        X     = self._vectorizer.transform(processed)
        probs = self._classifier.predict_proba(X)       # shape (N, n_classes)
        classes = self._classifier.classes_

        results = {}
        for name, prob_row in zip(names, probs):
            best_idx  = int(prob_row.argmax())
            pred_path = classes[best_idx]
            confidence = float(prob_row[best_idx])

            # Apply boost rules the same way get_category_with_boost does
            if self._compiled_rules:
                boosted_path, boosted_conf = self._apply_boost(name, pred_path, prob_row, classes)
                pred_path  = boosted_path
                confidence = boosted_conf

            results[name] = (pred_path, confidence)
